...
class Validator
{
    ...
    private function validatePostalCodeExistence($postal_code)
    {
        $api_url = "https://api.zipaddress.net/?zipcode=" . urlencode($postal_code);
        $response = file_get_contents($api_url);
        $response_data = json_decode($response, true);
        return isset($response_data['code']) && $response_data['code'] === 200;
    }
    ...
    public function validate($data)
    {
        ...
        } elseif (!$this->validatePostalCodeExistence($data['postal_code'])) {
