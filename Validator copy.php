...
*/
        ...
        if (empty($data['postal_code'])) {
            ...
            $errors['postal_code'] = '郵便番号が正しくありません';
        } else {
            $master_data = $this->getPostalCodeData($clean_postal_code);
            if (empty($master_data)) {
                $errors['postal_code'] = '郵便番号は存在しません';
            }
        }
        ...
        if (empty($data['prefecture']) || empty($data['city_town'])) {
            $errors['address'] = '住所(都道府県もしくは市区町村・番地)が入力されていません';
        } elseif (mb_strlen($data['prefecture']) > 10) {
            $errors['address'] = '都道府県は10文字以内で入力してください';
        } elseif (mb_strlen($data['city_town']) > 50 || mb_strlen($data['building']) > 50) {
            $errors['address'] = '市区町村・番地もしくは建物名は50文字以内で入力してください';
        }
        ...
        if (empty($data['tel'])) {
            ...
            $errors['tel'] = '電話番号は12~13桁で正しく入力してください';
        }
...
*/
    ...
    private function getPostalCodeData($postal_code)
    {
        ...
        try {
            ...
            // error_log($e->getMessage());
            $errors['postal_code'] = '郵便番号の照合中にエラーが発生しました';
            return null;
        }
    }
}
