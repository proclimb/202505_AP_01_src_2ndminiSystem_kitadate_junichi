...
function paginationLinks(int $currentPage, int $totalPages, string $nameKeyword, ?string $sortBy, ?string $sortOrd, ?string $genderFlag, ?string $searchPref): string
{
    ...
    // 「デフォルト画面へ」ボタン（常に表示）
    $html .= "<a href=\"dashboard.php\" style=\"padding:6px 12px; text-decoration:none; color:#007bff;\">デフォルト画面へ</a>";

    // 総ページ数が1以下（＝ページ遷移不要）なら「デフォルト画面へ」リンクのみ表示
    /* if ($totalPages <= 1) {
        return "<div class=\"pagination\" style=\"width:80%; margin: 10px auto; text-align:center;\">
              <a href=\"dashboard.php\">デフォルト画面へ</a>
            </div>";
    } */
